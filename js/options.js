...
$(function () {
    ...
    var DEFAULT_SHORTCUT_FILLER = "Shortcut"
        ...
        , IMAGE_REFRESH_ICON = 'images/icons/refresh.png'
        , IMAGE_REMOVE_ICON = 'images/icons/bin.png'
    ...
    // Set omnibar keyword
    $('#omniboxKeyword').text(browser.i18n.getMessage("KEYWORD_OMNIBAR_TRIGGER"));
    ...
    function refreshShortcuts() {
        ...
        browser.storage.sync.get(null, function (data) {
            ...
            storageQuota = browser.storage.sync.QUOTA_BYTES;
            itemStorageQuota = browser.storage.sync.QUOTA_BYTES_PER_ITEM;
            countQuota = browser.storage.sync.MAX_ITEMS;
    ...
    function processMetaData(data) {
        ...
        if (shortcutVersion && shortcutVersion != metaData[SHORTCUT_VERSION_KEY]) {
            ...
            console.log(browser.i18n.getMessage("WARNING_SHORTCUT_VERSION_MISMATCH"));
            alert(browser.i18n.getMessage("WARNING_SHORTCUT_VERSION_MISMATCH"));
            console.log('Database version:', shortcutVersion);
    ...
    function setupShortcuts(data, completionBlock) {
        ...
        $('.refreshButton').find('img').attr('src', 'images/icons/loading.gif');
        $('#edit').fadeOut(ANIMATION_FAST, function () {
            $(this).html('').fadeIn(ANIMATION_FAST, function () {
                ...
                {
                    browser.storage.local.get(APP_FIRST_RUN_KEY, function (firstRun) {
                        if (browser.runtime.lastError) {		// Check for errors
                            console.log(browser.runtime.lastError);
                        }
                        else if (!firstRun[APP_FIRST_RUN_KEY])		// First run
                        {
                            ...
                            browser.storage.local.set(firstRun);

                            // Example shortcuts
                            addRow('brb', 'be right back');
                            addRow('@hi', 'Hi and welcome, how are you?');
                            addRow('hp', 'Happy birthday!');
    ...
    function addRow(shortcut, autotext, append) {
        if ($('tr').length >= adjustedCountQuota) {
            console.log(browser.i18n.getMessage("ERROR_OVER_ITEM_QUOTA"));
            showCrouton(browser.i18n.getMessage("ERROR_OVER_ITEM_QUOTA")
    ...
    function validateRow($input, callback) {
        ...
        if (itemSize >= itemStorageQuota) {
            console.log(browser.i18n.getMessage("ERROR_OVER_SPACE_QUOTA"));
    ...
    function saveShortcuts(completionBlock) {
        ...
        if (duplicates.length) {
            console.log(browser.i18n.getMessage("ERROR_DUPLICATE_ITEMS"));
            showModalPopup(browser.i18n.getMessage("ERROR_DUPLICATE_ITEMS")
                + '\n - ' + duplicates.join('\n - '));
            return false;
        }
        ...
        if (JSON.stringify(data).length >= storageQuota) {
            console.log(browser.i18n.getMessage("ERROR_OVER_SPACE_QUOTA"));
            showCrouton(browser.i18n.getMessage("ERROR_OVER_SPACE_QUOTA")
    ...
    function backupShortcuts() {
        showModalPopup(browser.i18n.getMessage("MESSAGE_BACKUP_WARNING") + " Continue?",
            function (response) {
                if (response) {
                        ...
                        browser.storage.sync.get(null, function (data) {
                            ...
                            else	// Save backup of shortcuts
                            {
                                ...
                                browser.storage.local.set(backup, function () {
                                    if (browser.runtime.lastError) {	// Check for errors
                                        console.log(browser.runtime.lastError);
                                        showCrouton(browser.i18n.getMessage("ERROR_BACKUP_FAILED"), 'red');
                                    }
                                    else {	// Show success
    ...
    function updateBackupTimestamp() {
        browser.storage.local.get(APP_BACKUP_TIMESTAMP_KEY, function (data) {
            if (browser.runtime.lastError) {	// Check for errors
                console.log(browser.runtime.lastError);
            }
    ...
    function promptEmergencyRestore() {
        showModalPopup(browser.i18n.getMessage("MESSAGE_EMERGENCY_RESTORE_WARNING"),
            function (response) {
                if (response) {
                    getEmergencyBackup(function (data)	// Restore using emergency backup
                    {
                        ...
                        browser.storage.sync.set(data[APP_EMERGENCY_BACKUP_KEY], function () {
                            ...
                            {
                                console.log(browser.runtime.lastError);
                                showCrouton(browser.i18n.getMessage("ERROR_RESTORE_FAILED"), 'red');
                            }
                            else 	// Show success
                            {
    ...
    function restoreShortcuts() {
        ...
        // Confirm restore
        showModalPopup(browser.i18n.getMessage("MESSAGE_RESTORE_WARNING") + " Continue?",
            function (response) {
                if (response) {
                    getLocalBackup(function (data)	// Restore using backup shortcuts
                    {
                        console.log("Restoring shortcuts: ", data[APP_BACKUP_KEY]);
                        browser.storage.sync.set(data[APP_BACKUP_KEY], function () {
                            if (browser.runtime.lastError) 	// Check for errors
                            {
                                console.log(browser.runtime.lastError);
                                showCrouton(browser.i18n.getMessage("ERROR_RESTORE_FAILED"), 'red');
    ...
    function portShortcuts() {
        showPortView(function (newShortcuts) {
            ...
            try {
                ...
                showCrouton(browser.i18n.getMessage("ERROR_IMPORT_INVALID_JSON"), 'red');
                return;
            }

            // Check if it's an array, has to be an object
            if ($.isArray(newShortcuts)) {
                showCrouton(browser.i18n.getMessage("ERROR_IMPORT_NOT_OBJECT"), 'red');
                return;
            }
            ...
            setupShortcuts(shortcuts, function (success) {
                ...
                if (success) {
                    showCrouton(browser.i18n.getMessage("MESSAGE_IMPORT_SUCCESS"), 'orange', true);
                } else {
                    showCrouton(browser.i18n.getMessage("ERROR_IMPORT_ADDING_ROWS"), 'red');
                }
    ...
    function showModalPopup(message, completionBlock, isConfirm) {
        ...
        $(document.createElement('div'))
            ...
            .append($(document.createElement('h2'))
                .text(browser.i18n.getMessage("TITLE_WARNING_POPUP"))
            )
        ...
        browser.storage.sync.get(null, function (data) {
            ...
            else	// Parse json and show
            {
                ...
                $(document.createElement('div'))
                    ...
                    .append($(document.createElement('h2'))
                        .text(browser.i18n.getMessage("TITLE_PORT_VIEW_POPUP"))
                    )
                    .append($(document.createElement('p'))
                        .html(browser.i18n.getMessage("TEXT_PORT_VIEW_POPUP"))
                    )
                    .append($(document.createElement('textarea'))
                        .attr('id', 'portJSON')
