...
(function($) {
  ...
  {
		browser.storage.sync.get(SHORTCUT_TIMEOUT_KEY, function (data)
		{
			// Check for errors
    ...
    var isAllCaps = (shortcut == shortcut.toUpperCase());   // Check for all caps
		...
		// Get shortcuts
		browser.storage.sync.get(shortcutKey, function (data)
		{
			// Check for errors
			if (browser.runtime.lastError) {
				console.log(browser.runtime.lastError);
      ...
      else if (shortcutKeyLowercase != shortcutKey)
      {
        ...
        browser.storage.sync.get(shortcutKeyLowercase, function (data)
        {
          // Check for errors
          if (browser.runtime.lastError) {
            console.log(browser.runtime.lastError);
  ...
  function setCursorPositionInNode(node, pos, win, doc)
  {
    ...
    var mo = moment();
    mo.locale(browser.i18n.getMessage('@@ui_locale'));
  ...
  {
    browser.storage.sync.get(SHORTCUT_VERSION_KEY, function (data)
    {
			// Check for errors
			if (browser.runtime.lastError) {
				console.log(browser.runtime.lastError);
        ...
        && data[SHORTCUT_VERSION_KEY] != APP_VERSION)   // If versions don't match up
      {
        ...
        var warning = browser.i18n.getMessage("WARNING_SHORTCUT_VERSION_MISMATCH")
          + '\n\n' + browser.i18n.getMessage("WARNING_SHORTCUT_DISABLED");
        console.log(warning);
  ...
  function showCrouton(message, autohide)
  {
    ...
    crouton.style['background-color'] = '#cc636e';
